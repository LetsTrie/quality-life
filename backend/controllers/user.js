...
const { Notification, Professional } = require('../models');
...
async function getLastResult(types, req, res, next, format) {
    ...
    if (msm) {
        ...
        if (format) {
            let t = moment(msm.date, 'DD-MM-YYYY')
                .format('L')
                .toString()
                .split('/');
...
exports.submitProfScale = asyncHandler(async (req, res, _next) => {
    ...
    const professional = await Professional.findById(assessment.prof);
    if (!professional) {
        return sendErrorResponse(res, 404, 'NotFound', {
    ...
    await NotificationService.scaleSubmittedByUser({
        ...
        assessmentName: formatSlugToTitle(assessment.assessmentSlug),
    });
