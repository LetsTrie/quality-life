...
const SeminarWorkshop = ({ navigation, route, ...props }) => {
...
await axios.post(`${BaseUrl}/prof/table/createSeminarWorkshop`, newData).then((res) => {
    if(res.data === 'success'){
...
await  axios.post(`${BaseUrl}/prof/table/delSeminarWorkshop`,deleteData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
...
const getData = async () => {
    ...
    await  axios.post(`${BaseUrl}/prof/table/getSeminarWorkshop`, userId).then((res) => {
        if (res.data.success === true) {
            setDataSource(res?.data?.seminarWorkshop);
        }
        setIsLoading(false);
        });
} ;
...
await  axios.post(`${BaseUrl}/prof/table/updateSeminarWorkshop`,editingData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
            return pre.map((data) => {
...
return (
<>
    <Text style={{fontSize: 15, color: '#fc5c65', marginLeft: 15}}>1.3 Seminar/ Workshop (Time in hour)</Text>
    <ScrollView horizontal={true} style={{marginBottom: 50,}}>
    <View style={[styles.block, { paddingBottom: 23 }]}>
    <Provider>
    <DataTable >
    <DataTable.Header>
        <DataTable.Title style={{width: 120, marginRight: 10}}>Seminar/Workshop</DataTable.Title>
        <DataTable.Title style={{width: 140, marginRight: 10}}>Title</DataTable.Title>
    ...
    return <>
        <DataTable.Row key ={data._id}>
            <DataTable.Cell style={{width: 120, marginRight: 10}}>{data.seminarWorkshop}</DataTable.Cell>
...
<View style={styles.centeredView}>
    ...
    onRequestClose={() => {
        ...
        <TextInput
            ...
            defaultValue={`${editingData?.seminarWorkshop}`}
            onChangeText={(e) => {
            setEditingData((pre) => {
                return { ...pre, seminarWorkshop: e };
            });
            }}
        />
        <TextInput
...
<View style={styles.centeredView}>
    ...
    onRequestClose={() => {
        ...
        <TextInput
            ...
            placeholder='Seminar/Workshop'
            style={{ marginBottom: 5 ,borderRadius: 5}}
            onChangeText={(text) => createChangeHandler(text, 'seminarWorkshop')}
        />
        <TextInput
            autoCapitalize='none'
            autoCorrect={false}
...
export default connect(mapStateToProps)(SeminarWorkshop);