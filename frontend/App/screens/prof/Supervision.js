...
const Supervision = ({ navigation, route, ...props }) => {
...
await axios.post(`${BaseUrl}/prof/table/createSupervision`, newData).then((res) => {
    if(res.data === 'success'){
...
await  axios.post(`${BaseUrl}/prof/table/delSupervision`,deleteData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
...
const getData = async () => {
    ...
    await  axios.post(`${BaseUrl}/prof/table/getSupervision`,userId).then((res) => {
        if (res.data.success === true) {
            setDataSource(res?.data?.supervision);
        }
        setIsLoading(false);
        });
} ;
...
await  axios.post(`${BaseUrl}/prof/table/updateSupervision`,editingData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
            return pre.map((data) => {
...
return (
<>
    <Text style={{fontSize: 15, color: '#fc5c65', marginLeft: 15}}>1.2 Supervision (Time in hour)</Text>
    <ScrollView horizontal={true} style={{marginBottom: 50,}}>
    <View style={[styles.block, { paddingBottom: 23 }]}>
    <Provider>
    <DataTable >
    <DataTable.Header>
        <DataTable.Title style={{width: 140, marginRight: 10}}>Supervision Received/Facilitated</DataTable.Title>
    ...
    return <>
        <DataTable.Row key ={data._id}>
            <DataTable.Cell style={{width: 140, marginRight: 10}}>{data.supervisionR_F}</DataTable.Cell>
...
<View style={styles.centeredView}>
    ...
    onRequestClose={() => {
        ...
        <TextInput
            ...
            autoCorrect={false}
            name='supervisionR_F'
            placeholder='Supervision Received/Facilitated'
            style={{ marginBottom: 5 ,borderRadius: 5}}
            defaultValue={`${editingData?.supervisionR_F}`}
            onChangeText={(e) => {
            setEditingData((pre) => {
                return { ...pre, supervisionR_F: e };
            });
            }}
        />
        <TextInput
...
<View style={styles.centeredView}>
    ...
    onRequestClose={() => {
        ...
        <TextInput
            ...
            placeholder='Supervision Received/Facilitated'
            style={{ marginBottom: 5 ,borderRadius: 5}}
            onChangeText={(text) => createChangeHandler(text, 'supervisionR_F')}
        />
        <TextInput
            autoCapitalize='none'
            autoCorrect={false}
