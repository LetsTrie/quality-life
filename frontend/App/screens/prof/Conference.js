...
const Conference = ({ navigation, route, ...props }) => {
...
await axios.post(`${BaseUrl}/prof/table/createConference`, newData).then((res) => {
    if(res.data === 'success'){
...
await  axios.post(`${BaseUrl}/prof/table/delConference`,deleteData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
...
const getData = async () => {
    ...
    await  axios.post(`${BaseUrl}/prof/table/getConference`, userId).then((res) => {
        if (res.data.success === true) {
            setDataSource(res?.data?.conference);
        }
        setIsLoading(false);
        });
} ;
...
await  axios.post(`${BaseUrl}/prof/table/updateConference`,editingData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
            return pre.map((data) => {
...
return (
<>
    <Text style={{fontSize: 15, color: '#fc5c65', marginLeft: 15}}>1.4 Conference ( last 6 month)</Text>
