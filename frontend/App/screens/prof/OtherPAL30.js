...
const OtherPAL30 = ({ navigation, route, ...props }) => {
...
await axios.post(`${BaseUrl}/prof/table/createOtherPAL30`, newData).then((res) => {
    if(res.data === 'success'){
...
await  axios.post(`${BaseUrl}/prof/table/delOtherPAL30`,deleteData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
...
const getData = async () => {
    ...
    await  axios.post(`${BaseUrl}/prof/table/getOtherPAL30`, userId).then((res) => {
        if (res.data.success === true) {
            setDataSource(res?.data?.otherPAL30);
        }
        setIsLoading(false);
        });
} ;
...
await  axios.post(`${BaseUrl}/prof/table/updateOtherPAL30`,editingData).then((res) => {
    if (res.data === 'success') {
        setDataSource((pre) => {
            return pre.map((data) => {
...
return (
<>
    <Text style={{fontSize: 15, color: '#fc5c65', marginLeft: 15}}>Please answer following questions on the basis of your activities in last 30 days.</Text>
    <ScrollView horizontal={true} style={{marginBottom: 50,}}>
    <View style={[styles.block, { paddingBottom: 23 }]}>
    <Provider>
    <DataTable >
    <DataTable.Header>
        <DataTable.Title style={{width: 160, marginRight: 10}}>Question</DataTable.Title>
        <DataTable.Title style={{width: 100, marginRight: 10}}>Answer</DataTable.Title>
    ...
    return <>
        <DataTable.Row key ={data._id}>
            <DataTable.Cell style={{width: 160, marginRight: 10}}>{data.question}</DataTable.Cell>
            <DataTable.Cell style={{width: 100, marginRight: 10}}>{data.answer}</DataTable.Cell>
...
<View style={styles.centeredView}>
    ...
    onRequestClose={() => {
        ...
        <TextInput
            ...
            defaultValue={`${editingData?.question}`}
            onChangeText={(e) => {
            setEditingData((pre) => {
                return { ...pre, question: e };
            });
            }}
        />
        <TextInput
            ...
            placeholder='Answer'
            style={{ marginBottom: 5 ,borderRadius: 5}}
            defaultValue={`${editingData?.answer}`}
            onChangeText={(e) => {
            setEditingData((pre) => {
                return { ...pre, answer: e };
            });
...
<View style={styles.centeredView}>
    ...
    onRequestClose={() => {
        ...
        <TextInput
            ...
            placeholder='Question'
            style={{ marginBottom: 5 ,borderRadius: 5}}
            onChangeText={(text) => createChangeHandler(text, 'question')}
        />
        <TextInput
            autoCapitalize='none'
            autoCorrect={false}
            ...
            placeholder='Answer'
            style={{ marginBottom: 5 ,borderRadius: 5}}
            onChangeText={(text) => createChangeHandler(text, 'answer')}
        />
        <View style={styles.buttonDiv}>
        <Pressable
            style={[styles.button, styles.buttonYes]}
...
export default connect(mapStateToProps)(OtherPAL30);